...
_1. The current MSigDB v6.1 gmt files were downloaded manually since programmatically download them it is not feasible._
_2. Human Gene symbols were mapped to Mouse Gene symbols, using biomaRt ensembl package (20180110)._
...
There are three data frames (tibbles) this package. The `msigdf.human` data frame has columns for each MSigDB collection (c1-7 and hallmark), sub-collection (like C5 cc, bp and mf), each gene set, and gene symbol, where each row is a single gene symbol. The `msigdf.mouse` data frame has the same structure for mouse orthologs. The `msigdf.urls` data frame links the name of the gene set to the URL on the Broad's website.
